...
public class GameController {
    ...
    public void checkIfinJailBeforeMoving(){
        ...
        if(!currentPlayer.isInJail()) {
            ...
            if(currentPlayer.getDoubleThrowNum()>2){
                ...
                int boardLength = board.getFields().length;
                sumOfDice = (boardLength-lastField+10)%boardLength;
    ...
    public String[][] getChoices(Player player){
        ...
        if (playerInJail && currentField instanceof JailField) {

            if (currentPlayer.getCurrentTurn() > currentPlayer.getJailTurn() && currentPlayer.isInJail()) {
                String option = languageCollection.getMenu()[34] + ",8";
                choiceList = addToStringArray(choiceList, option);
            }
            if (player.getJailCardStatus()) {
                String option = languageCollection.getMenu()[33] + ",1";
                choiceList = addToStringArray(choiceList, option);
            }
            if (currentPlayer.getCurrentTurn() > currentPlayer.getJailTurn()) {

                String option = languageCollection.getMenu()[30] + " " + ((JailField) currentField).getBailAmount() + ",2";
                choiceList = addToStringArray(choiceList, option);
        ...
        if (player.getJailCardStatus()) {
            String option = languageCollection.getMenu()[32] + ",4";
            choiceList = addToStringArray(choiceList, option);
        }
        ...
        if (playerFieldRelationController.getUnpawnedFieldsWithNoHousesByPlayer(player).length > 0) {
        ...
        String option = languageCollection.getMenu()[36] + ",0";
    ...
    public void playerOptions(String[][] choices, Player player){
        ...
        String choiceList = viewController.getUserSelection(languageCollection.getMenu()[29] + " " + currentPlayer.getName(), choiceOptions);
    ...
    private void pawnProperty(Player player){
        ...
        if (field instanceof Ownable){
            ((Ownable) field).setPawnedStatus(true);
            tradecontroller.transferAssets(player,((Ownable) field).getPrice()/2);
